...
import { PrismaClient, type WorkflowPreset } from '@prisma/client';
...
async function jira<T>(path: string, params: Record<string, string> = {}): Promise<T> {
...
function mapStatus(jiraStatus: string, preset: WorkflowPreset): { status: string; viaDefault: boolean } {
  const lower = jiraStatus.toLowerCase();
  const done = ['done', 'closed', 'resolved', 'completed', 'cancelled', 'canceled', "won't do", 'wont do'];

  if (preset === 'generic') {
    if (done.some((k) => lower.includes(k))) return { status: 'Done', viaDefault: false };
  ...
  if (preset === 'design') {
    if (done.some((k) => lower.includes(k))) return { status: 'Done', viaDefault: false };
    if (lower.includes('approv'))   return { status: 'Approved', viaDefault: false };
    if (lower.includes('review'))   return { status: 'In Review', viaDefault: false };
    if (lower.includes('progress') || lower.includes('doing')) return { status: 'In Progress', viaDefault: false };
    return { status: 'Todo', viaDefault: true };
  }

  if (done.some((k) => lower.includes(k))) return { status: 'Done', viaDefault: false };
  if (lower.includes('test') || lower.includes('qa') || lower.includes('uat')) return { status: 'Testing', viaDefault: false };
  if (lower.includes('review')) return { status: 'In Review', viaDefault: false };
  if (lower.includes('progress') || lower.includes('doing') || lower.includes('development') || lower.includes('selected'))
    return { status: 'In Progress', viaDefault: false };
  return { status: 'Todo', viaDefault: true };
...
function inferPreset(name: string, projectTypeKey: string): WorkflowPreset {
  if (projectTypeKey === 'software') return 'engineering';
...
async function main(): Promise<void> {
  ...
  const jiraProjects = await jira<Array<{ key: string; name: string; projectTypeKey: string }>>(
    '/rest/api/3/project',
  );
  ...
  const hist = new Map<string, { jiraStatus: string; preset: WorkflowPreset; count: number; mapped: string; viaDefault: boolean; sampleProjects: Set<string> }>();
  ...
  for (const r of rows) {
    ...
    console.log(
      `${String(r.count).padStart(5)}  ${r.preset.padEnd(11)}  ${r.jiraStatus.padEnd(21)}  →  ${r.mapped.padEnd(17)}  ${flag}  ${sample}`,
    );
  }
