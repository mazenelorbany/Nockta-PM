...
const maintenance_scheduler_1 = require("../src/modules/maintenance/maintenance.scheduler");
...
async function main() {
    ...
    const log = new common_1.Logger('purge-loadtest');
    ...
    const scheduler = app.get(maintenance_scheduler_1.MaintenanceScheduler);
    ...
    for (let i = 0; i < opts.count; i += 1) {
        ...
        if (thumb200Key)
            await storage.putBuffer(thumb200Key, stubBody, 'image/webp');
        if (thumb800Key)
            await storage.putBuffer(thumb800Key, stubBody, 'image/webp');
        ...
        if (thumb200Key)
            seededKeys.push(thumb200Key);
        if (thumb800Key)
            seededKeys.push(thumb800Key);
        if ((i + 1) % 100 === 0)
            log.log(`  seeded ${i + 1}/${opts.count}`);
    }
    ...
    log.log(`Purge complete: ${purged}/${opts.count} rows in ${purgeMs}ms ` +
...
//# sourceMappingURL=storage-purge-loadtest.js.map