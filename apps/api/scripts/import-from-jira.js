...
const jira_import_service_1 = require("../src/modules/import/jira-import.service");
...
async function main() {
    ...
    console.log(`  filter          ${PROJECT_FILTER.length > 0 ? PROJECT_FILTER.join(', ') : '(all visible projects)'}`);
    console.log(`  dry run         ${DRY_RUN ? 'YES' : 'no'}`);
    console.log();
    const prisma = new client_1.PrismaClient();
    const runs = new import_runs_service_1.ImportRunsService(prisma);
    const service = new jira_import_service_1.JiraImportService(prisma, runs);
    const admin = await prisma.user.upsert({
        where: { email: ADMIN_EMAIL },
        update: {},
        create: { email: ADMIN_EMAIL, name: 'Admin', kind: 'internal', companyRole: 'Admin' },
        select: { id: true },
    });
    ...
    for (const jp of filtered) {
        ...
        const runId = await runs.start({
            ...
            sourceRef: jp.key,
            mappingSnapshot: { projectKey: jp.key, mapping: {} },
        });
        try {
...
//# sourceMappingURL=import-from-jira.js.map